...
export function DashboardCharts({ charts }: DashboardChartsProps) {
  return (
    <div className="grid min-w-0 grid-cols-1 gap-6 xl:grid-cols-2">
      <ChartCard title="연도별 교육 수" description="선택된 조건 기준 교육 건수">
        {charts.byYear.length === 0 ? <EmptyChart /> : <ChartContainer data={charts.byYear} />}
      </ChartCard>

      <ChartCard title="사업별 교육 수" description="사업별 운영 교육 건수">
      ...
      <ChartCard title="사업별 수료자 수" description="사업별 누적 수료자 수">
        {charts.completionByProject.length === 0 ? (
          <EmptyChart />
        ) : (
          <ChartContainer data={charts.completionByProject} />
        )}
      </ChartCard>

      <ChartCard title="담당자별 교육 수" description="담당자별 운영 교육 건수">
        {charts.byManager.length === 0 ? (
          <EmptyChart />
        ) : (
          <ChartContainer data={charts.byManager} />
        )}
      </ChartCard>
    </div>
  );
